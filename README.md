...
- save() method can return that primary key id value which is generated by hibernate and we can see it by
  ...
  persist() will never give any value back to the client, it return void 
...
- save() work even out of transaction	boundry but persist() will not work out of transaction boundary if used changes not reflected.
  
- in case if we setting pk manuuly then both save and persist is used save an entity objct into database ,
  it will throw an exception if an entity already exist with given identifier
